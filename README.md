...
- **简化的删除策略**：采用简化的叶子节点下溢处理方案，提高代码可维护性
...
- **分裂算法**：二次分裂 (Quadratic Split)
  - PickSeeds：选择最差的种子对
  - PickNext：选择最适合分配的条目
- **删除算法**：采用简化的叶子节点下溢处理策略（详见 [删除策略说明](docs/DELETION_STRATEGY.md)）
- **搜索算法**：递归搜索，支持范围查询
...
- **原论文方案**：复杂的多层次 CondenseTree 算法，处理所有节点的下溢
- **简化方案**：仅处理叶子节点下溢，通过重新插入解决，逻辑更简单

详细对比和使用建议请参阅：📋 [docs/DELETION_STRATEGY.md](docs/DELETION_STRATEGY.md)
