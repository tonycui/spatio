...
mod tests {
    ...
    fn test_delete_debug_detailed() {
        ...
        for i in 0..10 {
            ...
            println!(
                "Before deletion - Entry {}: found = {}",
                i,
                search_results.contains(&i.to_string())
            );
        }
    ...
    fn test_delete_with_underflow() {
        ...
        for &data in &remaining_data {
            ...
            assert!(
                search_results.contains(&data.to_string()),
                "Entry {} should still be findable after deletions",
                data
            );
        }
    ...
    fn test_simplified_underflow_handling() {
        ...
        for (id, _) in &geometries {
            if *id == 2 {
                ...
                let search_results = rtree.search_bbox(&Rectangle::new(0.0, 0.0, 15.0, 2.0));
                assert!(
                    search_results.contains(&id.to_string()),
                    "Entry {} should still be found after underflow handling",
                    id
                );
            }
        }
    ...
    fn test_reinsert_correctness() {
        ...
        for &expected in &expected_remaining {
            assert!(
                wide_search.contains(&expected.to_string()),
                "Entry {} should be in wide search results",
                expected
            );
        }
        assert!(
            !wide_search.contains(&"2".to_string()),
            "Deleted entry 2 should not be in wide search results"
        );
    ...
    fn test_mbr_update_after_deletion() {
        ...
        let search_results = rtree.search_bbox(&Rectangle::new(0.0, 0.0, 15.0, 15.0));
        assert!(
            search_results.contains(&"1".to_string()),
            "Entry 1 should still exist"
        );
        assert!(
            search_results.contains(&"3".to_string()),
            "Entry 3 should still exist"
        );
        assert!(
            search_results.contains(&"4".to_string()),
            "Entry 4 should still exist"
        );
        assert!(
            !search_results.contains(&"2".to_string()),
