...
impl RTree {
    ...
    pub fn search(&self, geometry: &Geometry) -> Vec<GeoItem> {
        let bbox = geometry_to_bbox(geometry);
        let mut results = Vec::new();
        
        if let Some(root) = self.root_ref() {
            self.search_recursive(root, &bbox.unwrap(), geometry, &mut results);
        }
    ...
    /// 递归搜索 - 遵循论文Search算法
    fn search_recursive(&self, node: &Node, query: &Rectangle, geometry: &Geometry, results: &mut Vec<GeoItem>) {
        // S1: 搜索子树
...
mod tests {
    ...
    fn test_search_with_geometry() {
        ...
        // 搜索相交的几何体
        let results = rtree.search( &query_polygon);
    ...
    fn test_search_polygon_intersection() {
        ...
        let results = rtree.search( &query_poly);
